...
services: 
  tre-ui:    # Matching the name of the service in the original TRE's compose manifest file
...
You can use the template from the `DeploymentStack/TRE/.env.example` file in the repository as a reference.
  ...
  <tbody>
    ...
    <Table.Tr>
      ...
      <Table.Td>
        ...
        e.g., [OMOP CDM](/explanations/concepts/omop) data. Camunda will use these credentials to create the
...
<Callout type="info">
  ...
  <Callout >
    ...
    - Use the URL of your remote KeyCloak server in the `.env` file as `TRE_KEYCLOAK_URL`.
    - You can build the realm configuration for the TRE layer from scratch, but we don't recommend it.
  </Callout>
...
<Table>
  ...
  <tbody>
    <Table.Tr>
      ...
      <Table.Td>
        The URL of the TRE Layer’s Keycloak server, for example, https://tre-keycloak.example.com.
      </Table.Td>
    </Table.Tr>
    <Table.Tr>
      <Table.Td>`TreApiPublicUrl`</Table.Td>
      <Table.Td>
        The public URL for the TRE Agent API. For example, https://my-TRE-host.example.com.
      </Table.Td>
    </Table.Tr>
    <Table.Tr>
      <Table.Td>`TRES3ConsoleUrl`</Table.Td>
      <Table.Td>
        Where the S3 TRE UI is hosted. For example, https://s3-tre-ui.example.com.
      </Table.Td>
    </Table.Tr>
    <Table.Tr>
      <Table.Td>`TreKeyCloakClientUIRedirectURL`</Table.Td>
      <Table.Td>
        The URL for the TRE UI's Keycloak redirect URL. For example, https://tre-keycloak.example.com/.
      </Table.Td>
    </Table.Tr>
    <Table.Tr>
      <Table.Td>`EgressKeyCloakClientUIRedirectURL`</Table.Td>
      <Table.Td>
        The URL for the Egress Layer's Keycloak redirect URL. For example, https://egress-keycloak.example.com/.
      </Table.Td>
    </Table.Tr>
    <Table.Tr>
      <Table.Td>`KeycloakFullURL`</Table.Td>
...
If you have not set this up yet, setup a TES backend, e.g., Funnel (with the installation guide [here](/guides/manage-tre/tes-backend/install-funnel)), and configure it to use the TRE S3 storage of this deployment.
...
<Table>
  ...
  <tbody>
    <Table.Tr>
      ...
      <Table.Td>Where TES backend is hosted. For example: `http://my-tes-backend.example.com/tasks`.</Table.Td>
    </Table.Tr>
  </tbody>
</Table>
...
- [Connect TRE to Data Egress](/guides/manage-tre/connect-egress)
...
| `volumes`     | Shared volumes that the containers can use to share data among themselves. The [TRE Agent](/explanations/how-5s-tes-works/tre-agent) always adds `/tmp` to this list before submission so workloads have a writable temporary directory (required on hardened TES backends such as TESK). Additional volumes from the request are preserved. This is applied at dispatch time, so it is not reflected in the task returned by the TES endpoints. |
...
A `Project` tag must contain the name of an approved [project](/explanations/how-5s-tes-works/entities#projects) in the submission layer
...
The `command` field is an argv array: `command[0]` must be the program to execute, followed by its arguments. See [Writing TES-compliant containers](/explanations/concepts/writing-tes-containers) for guidance on how this differs between TES backends such as Funnel and TESK.

This tells the TES engine in each TRE to load the [image](https://docs.docker.com/get-started/docker-concepts/the-basics/what-is-an-image/) of the container at the specified URL.
